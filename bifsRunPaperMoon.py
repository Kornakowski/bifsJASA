...
This code reproduces moon examples in the paper.
...
arr = np.array(cleanimg.getdata(), dtype=np.uint8)
nfield = np.resize(arr, (cleanimg.size[1], cleanimg.size[0]))
out = field
cleanimg = Image.fromarray(out, mode='L')
...
for i in range(panels):
    plotTitles.append('recon' + str(labelvals[i]))

parimgs = []
for i in range(panels):
    parimgs.append('parfn' + str(labelvals[i]))
