...
def c2g(img):
    """Generate rescaled image from input image from 0 to 256.
...
def c2gAdjust(img, minv, maxv):
    """Generate rescaled image between specified range minv to maxv.
    ...
    img : numpy.ndarray
        Image to be rescaled as 0 to 256.
    minv: int
        minimum value to rescale image to.
    maxv: int
        maximum value to rescale image to.
...
def genFSdata(imgarr, noiseSD=1.0, seedval=0):
    """Generate Fourier Space data adding Gaussian noise in image space.
    ...
    noise : float numpy.ndarray
        Output of just the noise image
    ...
    logknoiseSD : float
        Standard deviation of the log of the absolute noise in Fourier space.
    knoiseMean : float
        Mean of the absolute values of the noise in Fourier space.
    logknoiseMean : float
        Mean of the log of the absolute valuese of the noise in Fourier space.
...
def genFSdataTdist(imgarr, noiseSD=1.0, tdf=3, seedval=0):
    """Generate FS data from image (imgarr) adding t-distributed noise in image space.
    ...
    noiseSD : float, optional
        Standard deviation of noise to be added in image space
    tdf : int, optional
        Degrees of freedom for t-distributed noise. The default is 3
    seedval : int, optional
        Value for the random seed. The default is 0.
...
def bessd(x, limval=700.0):
    ...
    Specifically find ratio of modified Bessel function of first kind order 1
...
def flatten(x, cntrvals):
    """Flatten Fourier space in the set of indices cntrvals to lowest value on \
    the perimeter.
...
def pwr_ratio(mat1, mat2):
    """
    Return power ratio of two matrices across indices, but ignoring (0,0) freq.
...
def gauss_gauss_post(r, sig, m, s, zc=False):
    ...
    Bayes -- calculates posterior estimates at a single location in
    Fourier space
...
def gauss_gauss_n_post(r, sig, m, s, nval, zc=False):
    """Generate "weighted Gaussian posterior" for modulus in k-space.

    i.e. for mean of the modulus from prior and likelihood using conjugate
    Bayes, where the prior is weighted as nvals of observations -- calculates 
    posterior estimates at a single location in Fourier space
...
def exp_gauss_post(r, sig, m):
    ...
    prior and Gaussian likelihood -- calculates posterior estimates at a
    single location in Fourier space
...
def gauss_rice_post(r, sig, m, s, its=10):
    ...
    and Rician likelihood -- calculates posterior estimates at a single 
...
def exp_rice_post(r, sig, m, its=10):
    ...
    i.e. for mean of the modulus from prior and likelihood for Exponential
    prior and Rician likelihood -- calculates posterior estimates at a single
    location in Fourier space.
...
def expsq_rice_post(r, sig, m, its=100):
    ...
    i.e. for mean of the modulus from prior and likelihood for exponential
    square prior and Rician likelihood -- calculates posterior estimates at a
    single location in Fourier space
...
def linsc(bvec, x, exponent=1):
    ...
    x : float numpy.ndarray
        Distances from center of Fourier space for each index.
    exponent : float/int
        Exponent of inverse decay. Default value = 1.
...
def pltout(Image, imgset, outdir):
    """
    Helper function to output set of images.
